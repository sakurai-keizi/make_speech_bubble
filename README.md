...
| オプション | 説明 | 既定値 |
|---|---|---|
| `--seed` | 揺れ方を決める乱数シード（同じ値なら毎回同じ絵） | `7` |
| `--wobble` | 輪郭の揺れの強さ | `1.0` |
...
- **手書き風**: 楕円の輪郭を点列にし、整数次の正弦波（低周波ノイズ）で半径を揺らして「ゆらゆら」した閉曲線を作成。同じパスを少しずつズラして数回重ね描き（sketchy）することでペン入れ風の線にしています。`--seed` で揺れ方が決まり、再現性があります。
